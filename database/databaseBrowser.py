...
class DatabaseByFiles(DatabaseTabTemplate):
    ...
    def onConfirm(self):
        ...
        if self.ComboBoxDevice.currentIndex() != 0:
            ...
            for index, device in enumerate(self.devices):
                ...
                self.DrawData(device.id, offset=index / (self.devices.count() + 1), index=index,
                              max_count=(self.devices.count() + 1))
                # self.plot.draw()
                ...
                # self.plot2.draw()
        cur_ylim = self.axes1.get_ylim();
        self.axes1.set_ylim([0, cur_ylim[1]])
        self.plot.draw()

    def showData(self):
        if self.ComboBoxDevice.currentIndex() == 0:
            QMessageBox.question(None, 'Warning', 'You have to select a single device to display images.',
                                 QMessageBox.Ok)
            return
    ...
    def DrawData(self, device_id, offset=0, index=0, max_count=1):
        ...
        thread.sig.connect(lambda thread: self.DrawData2(year, month, day, thread.query, offset=offset, index=index,
                                                         max_count=max_count))
    ...
    def DrawData2(self, year, month, day, query, offset=0, index=0, max_count=1):
        ...
        if year == 0:
            ...
            self.axes1.set_title("%d. %s %d" % (
            day, ["Jan", "Feb", "Mar", "Apr", "May", "Jun", "Jul", "Aug", "Sep", "Oct", "Nov", "Dec"][month - 1], year))
    ...
    def doSaveFilelist(self):
        if self.ComboBoxDevice.currentIndex() == 0:
            QMessageBox.question(None, 'Warning', 'You have to select a single device to write file list.',
                                 QMessageBox.Ok)
            return
        self.SaveFileList(self.doSaveFilelist2)

    def doSaveFilelist2(self, count):
        QMessageBox.question(None, 'Saved', 'The file list filelist.txt has been saved with %d entries.' % count,
                             QMessageBox.Ok)
        return
...
class DatabaseByAnnotation(DatabaseTabTemplate):
    def __init__(self, parent):
        ...
        self.table.setHorizontalHeaderLabels(['Date', 'Tag', 'Comment', 'R', 'System', 'Cam', 'file'])
    ...
    def processLineEidtTimeDelta(self, text):
        reg = re.compile(
            "^(?:\D*(?P<year>\d{1,4})y)?(?:\D*(?P<month>\d{1,4})m)?(?:\D*(?P<day>\d{1,4})d)?(?:\D*(?P<hour>\d{1,4})H)?(?:\D*(?P<minute>\d{1,4})M)?(?:\D*(?P<second>\d{1,4})S)?")
        res = reg.match(text)
