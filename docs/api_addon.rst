...
.. note::
    The :doc:`addons` section demonstrates how the add-ons can be used and may serve as a good starting point
...
execution. For a good example of an add-on that uses the ``run`` function, refer to the :doc:`addon_track`.
...
- ``self.db`` is a :ref:`datafile` instance which gives access to the ClickPoints database. For details on the interface see :doc:`api`.
