...
Section "!${PRODUCT_NAME}" sec_app
  ...
  ;WriteRegStr HKCU "Software\Classes\Directory\shell\1${PRODUCT_NAME}" \
                   "" "${PRODUCT_NAME}"
  ;WriteRegStr HKCU "Software\Classes\Directory\shell\1${PRODUCT_NAME}" \
                   "icon" "$INSTDIR\${PRODUCT_ICON}"
  [% for scname, sc in ib.shortcuts.items() %]               
  ;WriteRegStr HKCU "Software\Classes\Directory\shell\1${PRODUCT_NAME}\command" \
                   "" "[[sc['target'] ]] [[ sc['parameters'] ]]"
  [% endfor %]                   
