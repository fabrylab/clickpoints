...
class DataFileExtended(DataFile):
    ...
    def initTimeStampRegEx(self) -> None:
        ...
        for regex in ast.literal_eval(self.getOption("timestamp_formats")):
            ...
            regex = regex.replace('%Y', r'(?P<Y>\d{4})', 1)
            regex = regex.replace('%y', r'(?P<y>\d{2})', 1)
            regex = regex.replace('%m', r'(?P<m>\d{2})', 1)
            regex = regex.replace('%d', r'(?P<d>\d{2})', 1)
            regex = regex.replace('%H', r'(?P<H>\d{2})', 1)
            regex = regex.replace('%M', r'(?P<M>\d{2})', 1)
            regex = regex.replace('%S', r'(?P<S>\d{2})', 1)
            regex = regex.replace('%f', r'(?P<f>\d{1,6})', 1)
            regex = regex.replace('*', '.*')
        ...
        for regex in ast.literal_eval(self.getOption("timestamp_formats2")):
            ...
            regex = regex.replace('%f', r'(?P<f>\d{1,6})', 1)
            # timestamp 2
            regex = regex.replace('%Y', r'(?P<Y2>\d{4})', 1)
            regex = regex.replace('%y', r'(?P<y2>\d{2})', 1)
            regex = regex.replace('%m', r'(?P<m2>\d{2})', 1)
            regex = regex.replace('%d', r'(?P<d2>\d{2})', 1)
            regex = regex.replace('%H', r'(?P<H2>\d{2})', 1)
            regex = regex.replace('%M', r'(?P<M2>\d{2})', 1)
            regex = regex.replace('%S', r'(?P<S2>\d{2})', 1)
            regex = regex.replace('%f', r'(?P<f2>\d{1,6})', 1)
            regex = regex.replace('*', '.*')
